	...
	function item_search()
{
	...
	{		
		...
		if(($this->input->post('payment_type') == lang('common_ebt') && $this->input->post('amount_tendered') > $this->sale_lib->get_ebt_total_amount_to_charge()) || ($this->input->post('payment_type') == lang('common_wic') && $this->input->post('amount_tendered') > $this->sale_lib->get_ebt_total_amount_to_charge()))
		{
	...
	function receipt($sale_id)
	{
		...
		{
			$cust_info=$this->Customer->get_info($customer_id);
			//$data['customer']=$cust_info->last_name.($cust_info->account_number==''  ? '':' - '.$cust_info->account_number);
			//edit by gomonx 
			$data['customer']=$cust_info->first_name.' '.$cust_info->last_name.($cust_info->account_number==''  ? '':' - '.$cust_info->account_number);
			$data['customer_company']= $cust_info->company_name;
			$data['customer_address_1'] = $cust_info->address_1;
	...
	{	
		...
		$data['ebt_total'] = $this->sale_lib->get_ebt_total_amount_to_charge();
  ...
  function cancel_sale()
  {
	...
	function downloadreceive($sale_id){
		//create Header
		$header_row = array('Item Id/Item Number/Product ID','Cost price','Quantity','Discount percentage');
		$rows[] = $header_row;
		
		$items = $this->Sale->get_sale_items($sale_id)->result();
		foreach($items as $i){
			$rows[] = array($i->item_id,$i->item_unit_price,$i->quantity_purchased,$i->discount_percent);
		}
		$this->load->helper('spreadsheet');
		array_to_spreadsheet($rows,'batch_receiving_Import - ' . $sale_id . '.'.($this->config->item('spreadsheet_format') == 'XLSX' ? 'xlsx' : 'csv'));
