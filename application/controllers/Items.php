...
class Items extends Secure_area implements Idata_controller
{
	...
	*/
	...
	function _get_item_data($item_id)
	{
      $this->load->helper('report');
		...
		if ($item_id == -1)
		{
            ...
            //fixbug is new item : Supplier Default = 'None' gomonX
	...
	//function gen Csv for bartender Barcode {MOCO Only} By gomonX
	function generate_csv_from_recv($recv_id,$enablespace=1){
		$this->load->model('Item_location');
		$this->load->model('Receiving');
                
                $Cur_receiv = $this->Receiving->get_info($recv_id)->row_array();
		//Create Header
		$header_row = array('Receive Date','ID','Barcode','Name','Qty','Sale Price','Expire Day');
		$rows[] = $header_row;
		
		foreach($this->Receiving->get_receiving_items($recv_id)->result() as $item){
                    $cur_item = $cur_item_info = $this->Item->get_info($item->item_id);
                    // if not Barcode Use Item_ID
                   if ($cur_item->item_number == ""){
                        $cur_item->item_number = $cur_item->item_id ;
                    }
                        $date =  date("ymd",strtotime($Cur_receiv['receiving_time']));
                    
                    // Add Space Lable
                    if($enablespace ==1){
                        if(fmod((int)$item->quantity_purchased,3) > 0){
                           $labelspace =  3 - fmod((int)$item->quantity_purchased,3);
                           $rows[] = array('','','','-',$labelspace,0,0);
                        }
                    }
                    
                    $rows[] = array($date,$cur_item->item_id,$cur_item->item_number,$cur_item->name,(int)$item->quantity_purchased,(int)$cur_item->unit_price,$cur_item->expire_days);	
                }
		
		$this->load->helper('spreadsheet');
		array_to_spreadsheet($rows,'Label_Receiving_Printing' . '.'.($this->config->item('spreadsheet_format') == 'XLSX' ? 'xlsx' : 'csv'));
	}
			...
			if ($this->input->post('locations'))
			{
				...
				{		        
					...
					if (isset($item_location_data['tax_names']))
					{
						...
						$tax_names = isset($item_location_data['tax_names']) ? $item_location_data['tax_names'] : array();
						$tax_percents = isset($item_location_data['tax_percents']) ? $item_location_data['tax_percents'] : array();
						$tax_cumulatives = isset($item_location_data['tax_cumulatives']) ? $item_location_data['tax_cumulatives'] : array();
	...
	function complete_excel_import()
	{
		...
		for ($i = 0; $i < $numRows -1; $i++)
		{
			...
			{
				$this->Item_location->save(array('location' => $location_at_store,'quantity' => NULL), $item_id,$current_location_id);
			}
