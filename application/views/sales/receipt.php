			...
			<div class="hidden-print search no-left-border">
				<ul class="list-inline print-buttons">
					...
					<?php if ($sale_id_raw != lang('sales_test_mode_transaction')) { ?>
						<li>
							<button class="btn btn-primary btn-lg hidden-print" id="creatpo_sheet_button" onclick="window.open('<?php echo site_url("sales/create_po/$sale_id_raw"); ?>', 'blank');" > <?php echo lang('common_create_po'); ?></button>
						</li>
						<?php } ?>	
					...
					?>	
						<li>
							<a class="btn btn-primary btn-lg hidden-print" id="downloadcsv_sheet_button" href="<?php echo site_url("sales/downloadreceive/$sale_id_raw"); ?>">Download Receipt</a>
						</li>
					<?php } ?>
			...
			<div class="panel-body panel-pad">
			        ...
			        <div class="col-md-4 col-sm-4 col-xs-12">
			          ...
			          <?php if(isset($customer)) { ?>
				        <ul class="list-unstyled invoice-address invoiceto" style="margin-bottom:2px;">
								<!--edit Receipt Customer Info by gomonx-->
								<li class="invoice-to"><?php echo lang('sales_invoice_to');?>:</li>
								<?php
									$removeName = $this->config->item('remove_customer_name_from_receipt');
									$removeCompany = $this->config->item('remove_customer_company_from_receipt');
								?>

								<?php if (!$removeName) {
									if(!empty($customer_company) && !$removeCompany){?>
										<li><?php echo lang('common_company').": ".$customer_company; ?></li>
									<?php }else{  ?>
									<li><?php echo lang('common_customer').": ".$customer; ?></li>
									
								<?php } } ?>
								<!-- end edit -->
								<?php if (!$removeCompany && $removeName) { ?>
								...
								<?php } ?>
									...
									<?php if (!$this->config->item('remove_customer_contact_info_from_receipt')) { ?>
										...
										<?php if(!empty($customer_tax_certificate)){ ?><li><?php echo lang('customers_tax_certificate'); ?> : <?php echo $customer_tax_certificate; ?></li><?php } ?>
									<?php } ?>
			    ...
			    <div class="invoice-table-content">
			      ...
			      <?php if($discount_exists) { ?>
						...
						<?php } ?>
									<div class="col-md-<?php echo $xs_col; ?> col-sm-<?php echo $xs_col; ?> col-xs-<?php echo $xs_col; ?> gift_receipt_element">      
					          <div class="invoice-content item-total pull-right"><?php echo to_currency($item['price']*$item['quantity']-$item['price']*$item['quantity']*$item['discount']/100); ?></div>
						      </div>
