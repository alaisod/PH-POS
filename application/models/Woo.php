...
class Woo extends  Ecom
{
	...
	public function save_item_from_phppos_to_ecommerce($item_data, $item_id)
	{
		...
		if (isset($item_data['images']) && $item_data['images'])
		{
			...
			{	
				$newimagefilename = str_replace('?timestamp','.jpg?timestamp',$item_data['images'][$k]['src']);
				$image_data =  array('src' => $newimagefilename, 'alt' => $item_data['images'][$k]['alt'], 'name' => $item_data['images'][$k]['name'], 'position' => $k);
		...
		if (!isset($item_data['ecommerce_product_id']) && !$item_data['ecommerce_product_id'])
		{
			...
			catch(Exception $e)
			{
				...
				// if new product & error : exit with out insert to ecommerce table add by gomonx
				return False ; 
			}
		}
		else
		{
			//Existing product
			try
			{//error put() >> chang to post() is OK.! ?? gomonx
				$woo_product = $woocommerce->post('products/'.$item_data['ecommerce_product_id'], $data);
			...
			{
				$this->log("*******".lang('common_EXCEPTION').": ".var_export($e->getMessage(),TRUE));
				// if  error : exit with out insert to ecommerce table add by gomonx
				return False ; 
			}
		...
		//ถ้า woo-respont ไม่สำเร็จ และหลุดมาถึงบรรทัดนี้ จะเกิด error คำสั่งนี้
	...
	function export_phppos_items_to_ecommerce()
	{
		...
		//write log //ย้ายไปบรรทัดที่: 962 >>  insert to ecommerce table
		...
		foreach($woo_items_chunked as $woo_items_chunk)
		{
			...
			if ($woo_response!= NULL && !empty($woo_response))
			{
				...
				{ 	//if woocommerce->post() Success!  add by gomonx
					if (!isset($woo_response['create'][$k]['error'])){
						//Add newly created woocommerce product in the ecommerce_products table
						...
						// writelog by gomonx
						$this->log(lang('save_item_to_ecommerce').': '.$woo_items[$k]['name']);
						...
						//if respons error: write log and Next>>    add by  gomonx
						$this->log("*******".lang('common_EXCEPTION').": ".var_export($woo_response['create'][$k]['error']['code'] . ' at Product : (' . $woo_items_chunk[$k]['sku'] . ') ' . $woo_items_chunk[0]['name'],TRUE));
					}
					$counter++;
