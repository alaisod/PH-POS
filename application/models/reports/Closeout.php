...
class Closeout extends Report
{
	...
	public function getData()
	{
		...
		if ($this->config->item('enable_customer_loyalty_system') && $this->config->item('loyalty_option') == 'advanced')
		{
			...
			$this->db->from('sales');
			$this->db->where('date(sale_time) BETWEEN '. $this->db->escape($this->params['start_date']). ' and '. $this->db->escape($this->params['end_date']).' and location_id IN('.$location_ids_string.')');
			$this->db->where('deleted', 0);
